...
if [ "$PGHOST" ]; then
    ...
    psql -U $PGUSER -h $PGHOST -d "coog_bi" -f $PENTAHO_HOME/scripts/Exec_date.sql

	if [ "$CHK_JCR" -eq "0" ]; then
     sed -i "s/PASSWORD 'password'/PASSWORD 'postgres'/" $PENTAHO_SERVER/data/postgresql/create_jcr_postgresql.sql
	 psql -U $PGUSER  -h $PGHOST -d $PGDATABASE -f $PENTAHO_SERVER/data/postgresql/create_jcr_postgresql.sql
	fi
	if [ "$CHK_HIBERNATE" -eq "0" ]; then
	 sed -i "s/PASSWORD 'password'/PASSWORD 'postgres'/" $PENTAHO_SERVER/data/postgresql/create_repository_postgresql.sql
	...
	if [ "$CHK_QUARTZ" -eq "0" ]; then
     sed -i "s/PASSWORD 'password'/PASSWORD 'postgres'/" $PENTAHO_SERVER/data/postgresql/create_quartz_postgresql.sql
